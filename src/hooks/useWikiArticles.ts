...
const getUrl = ({ date, country }: SearchParams): string => {
...
const useWikiArticles = () => {
  const fetchTopArticles = ({ country, date }: SearchParams) => {
    const formattedDate = format(new Date(date), "yyyy/MM/dd")
    const url = getUrl({ country, date: formattedDate })
  ...
  const fetchSummary = ({ title }: { title?: string }) => {
  ...
  const fetchMonthlyViews = ({ title }: { title?: string }) => {
